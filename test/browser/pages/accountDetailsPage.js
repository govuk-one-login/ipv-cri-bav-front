module.exports = class PlaywrightDevPage {
  ...
  async enterAccountNumber() {
    ...
    if (
      accNumInputVal.match(/^\d{6}$/) ||
      accNumInputVal.match(/^\d{7}$/) ||
      accNumInputVal.match(/^\d{8}$/)
    ) {
      accNumValid = 1;
    } else {
      accNumValid = 0;
  ...
  async verifySCFormat() {
    ...
    if (
      actualInputVal.match(/^\d{2}-\d{2}-\d{2}$/) ||
      actualInputVal.match(/^\d{6}$/) ||
      actualInputVal.match(/^\d{2}\s\d{2}\s\d{2}$/)
    ) {
      scValidFormat = 1;
    } else {
      scValidFormat = 0;
